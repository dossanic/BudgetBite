...
router.post("/price", async (req, res) => {
  ...
  if (!ingredients || !Array.isArray(ingredients) || ingredients.length === 0) {
    return res.status(400).json({
      error: "Vui lòng gửi một danh sách nguyên liệu!",
    });
  }

  const normalizedIngredients = ingredients.map(normalize);
  console.log("Normalized:", normalizedIngredients);
  ...
  console.log("Supabase data:", data); // ← thêm dòng này
  console.log("Supabase error:", error); // ← và dòng này
